...
public struct PeerConnectivityKeys {
    fileprivate static let CertificateListener = "CertificateRecievedListener"
}
...
public class PeerConnectionManager {
    ...
    public var sessionSecurityIdentity: [Any]?
    ...
    public var connectedServicePeers: [Peer] {
        servicesSessions.reduce([]) { peers, serviceSession -> [Peer] in
            Array(Set(peers + [serviceSession.servicePeer]))
        }
    }

    public var allAvailableSessions: [PeerSession] {
        [session] + servicesSessions
    }

    public var allAvailablePeers: [Peer] {
        servicesSessions.reduce(connectedPeers) { peers, serviceSession -> [Peer] in
            Array(Set(peers + [serviceSession.servicePeer] + serviceSession.connectedPeers))
        }
    }
    ...
    private let subService: ServiceType
    private let serviceDiscoveryInfo: [String: String]?

    internal var browser: PeerBrowser?
    private lazy var browserAssisstant: PeerBrowserAssisstant? = {
        PeerBrowserAssisstant(session: session, serviceType: serviceType,
                              eventProducer: browserViewControllerEventProducer)
    }()

    private let advertiser: PeerAdvertiser
    private lazy var advertiserAssisstant: PeerAdvertiserAssisstant? = {
        PeerAdvertiserAssisstant(session: session, serviceType: serviceType,
    ...
    internal var retyAttemptQueue: DispatchQueue?

    internal let responder: PeerConnectionResponder
    private let observer = MultiObservable<PeerConnectionEvent>(.ready)

    private let sessionObserver = Observable<SessionEventContainer>((nil, .none))
    ...
    public init(serviceType: ServiceType,
        ...
        sessionSecurityIdentity = identity
        sessionEncryptionPreference = encryptionPreference

        peer = Peer(peerID: MCPeerID(displayName: displayName), status: .currentUser,
...
extension PeerConnectionManager {
    ...
    private func updatePeersStatus(_ peer: Peer, session: PeerSession? = nil, status: Peer.Status) {
        ...
        foundPeers = foundPeers.map { $0 === peer ? updatedPeer : $0 }
