...
extension PeerSessionEventProducer: MCSessionDelegate {
    internal func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        let peer = Peer(peerID: peerID, status: Peer.Status(state: state))
        let event: PeerSessionEvent = .devicesChanged(peer: peer)
        let peerSession = PeerSession(session: session, sessionPeerStatus: .connected)

        observer.value = (peerSession, event)
    }
